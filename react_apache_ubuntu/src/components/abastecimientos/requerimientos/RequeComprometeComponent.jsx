...
export const RequeComprometeComponent = () => {
  ...
  const { T_reque_obs, C_reque, C_anipre, C_biesertipo, C_sf_dep, c_traba_dni, f_libre } = currentReque;
      ...
      let gastos = requeGasto.map((item) => {
        const presupuesto = item.presupuesto.map((presup) => {
          return {
            ...
            total_precompromiso: Number(parseFloat(presup.total_precompromiso).toFixed(2)),
          }
        });
    ...
    <div>
      ...
      <div className="d-flex justify-content-center mt-3">
        <Button
          ...
          disabled={!validatePrecomprometer()}
          onClick={onClicPrecomprometer}
        >
          {" "}
          {/* <FileDollarIcon className="me-1 thumbnail" /> */}
          <p className="m-0 p-0">Precomprometer</p>
        </Button>
      </div>
    </div>
  );
